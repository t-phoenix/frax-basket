...
async function main() {
  ...
  console.log("Signer Account: ", accounts)
  ...
  // const initTx = await controller.connect(owner).initialize([owner.address], [basicIssueModule.target], [], [] );
  // await initTx.wait(2);
  ...
  // await frxBASKET.waitForDeployment()
  ...
  // const addSetTrx = await controller.connect(owner).addSet(frxBASKET.target);
  // await addSetTrx.wait(2);
  // // Initialize BIM Module
  // const biminitTrx = await basicIssueModule.connect(owner).initialize(frxBASKET.target, '0x0000000000000000000000000000000000000000');
  // await biminitTrx.wait(2);
  ...
  //   const fraxTrx = await FRAX.connect(accounts[index]).approve(basicIssueModule.target, 20_000000000000000000n);
  //   await fraxTrx.wait(2);
  ...
  // const issueTrx = await basicIssueModule.connect(accounts[0]).issue(frxBASKET.target, 1_000000000000000000n, accounts[0].address)
  // await issueTrx.wait(2);
  ...
  // const addresouceTrx = controller.connect(owner).addResource(priceOracle.target, 1);
  // await addresouceTrx.wait(2);
  ...
  await setValuer.waitForDeployment();
  
  // Adding set Valuer to controller as module
  // SET VALUER MUST BE RESOURCE WITH ID - 2
  const addresouceTrx2 = await controller.connect(owner).addResource(setValuer.target, 2);
  // await addresouceTrx2.wait(2);
  ...
  await NAVModule.waitForDeployment()

  // Add NAV to Controller as module
  await controller.connect(owner).addModule(NAVModule.target);
  // Add NAV to SetToken as module
  const addNavTrx = await frxBASKET.connect(owner).addModule(NAVModule.target);
  await addNavTrx.wait(2);
  ...
  const navinitTrx = await NAVModule.connect(owner).initialize(frxBASKET.target, navIssuanceSettings)
  await navinitTrx.wait(2);
  ...
  const approveNavTrx = await FRAX.connect(owner).approve(NAVModule.target, 20_000000000000000000n);
  await approveNavTrx.wait(2);
  ...
  const addresouceTrx3 = await controller.connect(owner).addResource(integrationRegistry.target, 0);
  await addresouceTrx3.wait(2)
  ...
  await generalIndex.waitForDeployment()
  
  // Add module to Controller
  await controller.connect(owner).addModule(generalIndex.target);
  // Add module to Set Token
  const addModuleTrx = await frxBASKET.connect(owner).addModule(generalIndex.target);
  await addModuleTrx.wait(2)

  // Initialize Module
  const initgimTrx = await generalIndex.connect(owner).initialize(frxBASKET.target);
  await initgimTrx.wait(2)
  ...
  const setgimTrx = await generalIndex.connect(owner).setCoolOffPeriods(frxBASKET.target, [wfrxETH, FRAX, FXS], [1, 1, 1])
  await setgimTrx.wait(2)
  ...
  console.log("Set Valuer Contract: ", setValuer.target)
  ...
  await fraxswapIndexExchangeAdapter.waitForDeployment();
  ...
  const addintegtationTrx = await integrationRegistry.connect(owner).addIntegration(generalIndex.target, "FraxSwapRouter", fraxswapIndexExchangeAdapter);
  await addintegtationTrx.wait(2)
  ...
  const setExchangeTrx = await generalIndex.connect(owner).setExchanges(frxBASKET.target, [wfrxETH.target, FRAX.target, FXS.target], ["FraxSwapRouter", "FraxSwapRouter", "FraxSwapRouter"])
  await setExchangeTrx.wait(2);
  ...
  const startRebalanceTrx = await generalIndex.connect(owner).startRebalance(frxBASKET.target, [], [], [1000000000000000n, 1_000000000000000000n, 3_000000000000000000n], newMultiplier  ) //0.001wfrxETH, 1 FXS, 3 FRAX
